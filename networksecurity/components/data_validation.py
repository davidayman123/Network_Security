...
class DataValidation:
    ...
    def validate_number_of_columns(self, dataframe: pd.DataFrame) -> bool:
        try:
            ...
            raise NetworkSecurityException(e, sys) from e    
    def detect_data_drift(self, base_df, current_df,threshold=.05) -> bool:
        try:
            status=True
            report={}
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            if not validation_status:
                error_message = f"Train dataframe does not have required number of columns. Required: {len(self.schema_config['columns'])}, Found: {len(train_df.columns)}"
                raise Exception(error_message)
            validation_status = self.validate_number_of_columns(test_df)
            if not validation_status:
                error_message = f"Test dataframe does not have required number of columns. Required: {len(self.schema_config['columns'])}, Found: {len(test_df.columns)}"
                raise Exception(error_message)   
